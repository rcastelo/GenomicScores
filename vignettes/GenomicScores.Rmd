...
```{r}
gscores(gsco, GRanges(seqnames="chr22", IRanges(start=50967020:50967025, width=1)))
```
...
```{r}
gscores(gsco, gr2, summaryFun=max)
gscores(gsco, gr2, summaryFun=min)
gscores(gsco, gr2, summaryFun=median)
```
...
```{r}
gr1 <- gscores(gsco, gr1, quantized=TRUE)
gr1
```
...
```
```{r, echo=FALSE}
## temporary fix until the gwaswloc class from the gwascat package is fixed
eyersids <- readRDS(system.file("extdata", "eyersids.rds", package="GenomicScores"))
```
```{r}
eyersids
```
...
loc$MAF[width(loc) > 1] <- gscores(mafdb, loc[width(loc) > 1], type="nonsnvs")$AF
loc[1:3]
```
...
```{r}
pkgpcscoCDS <- score(gsco, origpcscoCDS)
pkgpcsco3UTRs <- score(gsco, origpcsco3UTRs)
```
...
of conservation, the quantization of _phastCons_ scores to one significant figure provides sufficient
