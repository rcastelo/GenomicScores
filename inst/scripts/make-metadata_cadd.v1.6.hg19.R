...
makeMetadata_CADD.v1.6.hg19 <- function()
{
  biocver <- "3.18"
  baseUrl <- "http://functionalgenomics.upf.edu/annotationhub/cadd/"
  meta <- .GenomicScoresMetadataFromUrl(baseUrl, "cadd.v1.6.hg19")
  n <- nrow(meta)
  data.frame(
...
metadata <- makeMetadata_CADD.v1.6.hg19()
write.csv(metadata, file="../extdata/metadata_cadd.v1.6.hg19.csv", row.names=FALSE)
