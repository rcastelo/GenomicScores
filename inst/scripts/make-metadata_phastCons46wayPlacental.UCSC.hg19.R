...
makeMetadata_phastCons46wayPlacental.UCSC.hg19 <- function()
{
  biocver <- "3.7"
  baseUrl <- "http://functionalgenomics.upf.edu/annotationhub/phastCons/"
  meta <- .GenomicScoresMetadataFromUrl(baseUrl, "phastCons46wayPlacental.UCSC.hg19")
  n <- nrow(meta)
  data.frame(
...
metadata <- makeMetadata_phastCons46wayPlacental.UCSC.hg19()
write.csv(metadata, file="../extdata/metadata_phastCons46wayPlacental.UCSC.hg19.csv", row.names=FALSE)
