...
makeMetadata_phastCons35way.UCSC.mm39 <- function()
{
  biocver <- "3.16"
  baseUrl <- "https://functionalgenomics.upf.edu/annotationhub/phastCons/"
  meta <- .GenomicScoresMetadataFromUrl(baseUrl, "phastCons35way.UCSC.mm39")
  n <- nrow(meta)
  data.frame(
    ...
    Tags=rep(paste(c("phastCons", "UCSC", "GScores"), collapse=":"), n))
}
metadata <- makeMetadata_phastCons35way.UCSC.mm39()
write.csv(metadata, file="../extdata/metadata_phastCons35way.UCSC.mm39.csv", row.names=FALSE)
