...
makeMetadata_CADD.v1.6.hg38 <- function()
{
  biocver <- "3.18"
  baseUrl <- "http://functionalgenomics.upf.edu/annotationhub/cadd/"
  meta <- .GenomicScoresMetadataFromUrl(baseUrl, "cadd.v1.6.hg38")
  n <- nrow(meta)
  data.frame(
    ...
    Tags=rep(paste("CADD", "UWashington", "GScores", sep=":"), n))
}
metadata <- makeMetadata_CADD.v1.6.hg38()
write.csv(metadata, file="../extdata/metadata_cadd.v1.6.hg38.csv", row.names=FALSE)
