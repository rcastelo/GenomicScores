...
foreach (chr=seqnames(Hsapiens)) %dopar% {
  cat(chr, "\n")
  wigToBigWig(file.path("hg38.100way.phyloP", sprintf("%s.phyloP100way.wigFix.gz", chr)), seqinfo=si)
}
...
saveRDS(refgenomeGD, file=file.path("hg38.100way.phyloP", "refgenomeGD.rds"))
...
## according to http://genome.ucsc.edu/cgi-bin/hgTrackUi?db=hg38&g=cons100way
...
foreach (chr=seqnames(Hsapiens)) %dopar% {
  cat(chr, "\n")
  tryCatch({
    rawscores <- import.bw(BigWigFile(file.path("hg38.100way.phyloP", sprintf("%s.phyloP100way.bw", chr))))
    ...
    metadata(obj) <- list(seqname=chr,
                          ...
                          data_pkgname="phyloP100way.UCSC.hg38",
                          qfun=.quantizer,
                          dqfun=.dequantizer,
                          ecdf=Fn,
                          max_abs_error=max.abs.error)
    saveRDS(obj, file=file.path("hg38.100way.phyloP", sprintf("phyloP100way.UCSC.hg38.%s.rds", chr)))
    rm(rawscores, obj)
