.GenomicScoresMetadataFromUrl <- function(baseUrl, track) {
  ...
  metadf <- data.frame(title=pcRDSfiles,
                       ...
                       description=sprintf("phyloP scores for %s on %s",
                                           organism(gd), chr),
                       rDataPath=rDataPath,
...
makeMetadata_phyloP100way.UCSC.hg38 <- function()
{
  baseUrl="http://functionalgenomics.upf.edu/annotationhub/phyloP/"
  meta <- .GenomicScoresMetadataFromUrl(baseUrl, "phyloP100way.UCSC.hg38")
  n <- nrow(meta)
  data.frame(
    ...
    Tags=rep(paste("phyloP", "GScores", sep=","), n))
}
metadata <- makeMetadata_phyloP100way.UCSC.hg38()
write.csv(metadata, file="../extdata/metadata_phyloP100way.UCSC.hg38.csv", row.names=FALSE)
