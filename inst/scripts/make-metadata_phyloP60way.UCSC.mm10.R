...
makeMetadata_phyloP60way.UCSC.mm10 <- function()
{
  biocver <- "3.7"
  baseUrl <- "http://functionalgenomics.upf.edu/annotationhub/phyloP/"
  meta <- .GenomicScoresMetadataFromUrl(baseUrl, "phyloP60way.UCSC.mm10")
  n <- nrow(meta)
  data.frame(
...
metadata <- makeMetadata_phyloP60way.UCSC.mm10()
write.csv(metadata, file="../extdata/metadata_phyloP60way.UCSC.mm10.csv", row.names=FALSE)
