...
makeMetadata_phastCons27way.UCSC.dm6 <- function()
{
  biocver <- "3.6"
  baseUrl <- "http://functionalgenomics.upf.edu/annotationhub/phastCons/"
  meta <- .GenomicScoresMetadataFromUrl(baseUrl, "phastCons27way.UCSC.dm6")
  n <- nrow(meta)
  data.frame(
...
metadata <- makeMetadata_phastCons27way.UCSC.dm6()
write.csv(metadata, file="../extdata/metadata_phastCons27way.UCSC.dm6.csv", row.names=FALSE)
