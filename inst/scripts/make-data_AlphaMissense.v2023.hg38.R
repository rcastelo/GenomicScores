...
## input: data.frame with POS, REF, ALT, SCORE for
## a given chromosome
## output: data.frame with POS, REF, ALT, SCORE
...
normalize_am_score_data <- function(dat) {
...
foreach (chr=seqlevels(allchrgr)) %dopar% {
  tryCatch({
    ...
    rawscores <- normalize_am_score_data(rawscores)
