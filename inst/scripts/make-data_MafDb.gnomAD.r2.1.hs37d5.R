...
path2vcfs <- "/projects_fg/GenomicScores/gnomad/genomes"
...
foreach (chr=seqlevels(vcfHeader)[1:23]) %dopar% {
  ...
  ## and this seems to be the actual strategy to store multi-allelic variants
...
for (chr in seqlevels(vcfHeader)[1:23]) {
  ...
  rr <- rowRanges(vcf)
  mcols(rr) <- NULL
  ## rrnames <- names(rr)[!duplicated(rr)]
  ## rr <- rr[!duplicated(rr)]
  gc()
