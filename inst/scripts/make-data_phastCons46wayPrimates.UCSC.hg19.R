...
## 10 primates. If you use these data on your own research
...
##
## $ rsync -avz --progress \
##     rsync://hgdownload.soe.ucsc.edu/goldenPath/hg19/phastCons46way/primates/ \
...
downloadURL <- "http://hgdownload.soe.ucsc.edu/goldenPath/hg19/phastCons46way/primates"
...
foreach (chr=seqnames(Hsapiens)) %dopar% {
  cat(chr, "\n")
  wigToBigWig(file.path("phastCons46wayPrimates", sprintf("%s.phastCons46way.primates.wigFix.gz", chr)), seqinfo=si)
}

pkgname <- "phastCons46wayPrimates.UCSC.hg19"
dir.create(pkgname)
...
nsites <- foreach (chr=seqnames(Hsapiens), .combine='c') %dopar% {
  cat(chr, "\n")
  tryCatch({
    rawscores <- import.bw(BigWigFile(file.path("phastCons46wayPrimates",
                                                sprintf("%s.phastCons46way.primates.bw", chr))))
