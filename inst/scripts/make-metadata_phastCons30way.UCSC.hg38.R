...
makeMetadata_phastCons46wayPlacental.UCSC.hg19 <- function()
{
  ...
  meta <- .GenomicScoresMetadataFromUrl(baseUrl, "phastCons30way.UCSC.hg38")
  n <- nrow(meta)
  data.frame(
...
write.csv(metadata, file="../extdata/metadata_phastCons30way.UCSC.hg38.csv", row.names=FALSE)
