.GenomicScoresMetadataFromUrl <- function(baseUrl, track) {
  ...
  metadf <- data.frame(title=rdsFiles,
                       ...
                       description=sprintf("linsight scores for %s on %s", organism(gd), chr),
                       rDataPath=rDataPath,
...
makeMetadata_linsight.UCSC.hg19 <- function()
{
  biocver <- "3.7"
  baseUrl <- "http://functionalgenomics.upf.edu/annotationhub/linsight/"
  meta <- .GenomicScoresMetadataFromUrl(baseUrl, "linsight.UCSC.hg19")
  n <- nrow(meta)
  data.frame(
    ...
    Tags=rep(paste("linsight", "GScores", sep=","), n))
}
metadata <- makeMetadata_linsight.UCSC.hg19()
write.csv(metadata, file="../extdata/metadata_linsight.UCSC.hg19.csv", row.names=FALSE)
