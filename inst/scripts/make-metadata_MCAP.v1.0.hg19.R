.GenomicScoresMetadataFromUrl <- function(baseUrl, track) {
  ...
  gd <- readRDS(gzcon(url(sprintf("%s%s/refgenomeGD.rds", baseUrl, track),
                          open="rb")))
  scRDSfiles <- AnnotationForge:::.getSubDirs(sprintf("%s%s", baseUrl, track))
  ...
  metadf <- data.frame(title=scRDSfiles,
                       ...
                       description=sprintf("MCAP scores v1.0 for %s on %s",
                                           organism(gd), chr),
                       rDataPath=rDataPath,
...
makeMetadata_MCAP.v1.0.hg19 <- function()
{
  ...
  data.frame(
    ...
    DataProvider=rep("Stanford", n),
    Maintainer=rep("Robert Castelo <robert.castelo@upf.edu>", n),
    RDataClass=rep("Rle", n),
    DispatchClass=rep("RDS", n),
    Location_Prefix=baseUrl,
    Tags=rep(paste("MCAP", "GScores", sep=","), n))
}
metadata <- makeMetadata_MCAP.v1.0.hg19()
write.csv(metadata, file="../extdata/metadata_MCAP.v1.0.hg19.csv", row.names=FALSE)
